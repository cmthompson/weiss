...
class RateRunout(object):
    ...
    def __init__(self, inits = None, _plot = False, _type = -1,parameters = None):
        self.initials = inits
    ...
    def dX_dt(self,X,time_in,model,minstep = 1E-3):
        global mwin , eVc  
        ...
        if self.parameters == None:
            ...
            k8 = 10#100 #reacgtion of PtO with PtH (assume 100)
            k9 = 10#100# reaction of PtOH with PtH (assume 100)
            g =  0
            ...
            (k1,_k1,k2, k3,k4 ,k5,k6 ,_k6,k7,k8 ,k9 ,g ,vOads,vOH,voffset) = self.parameters  ### su
    ...
    def RunOut(self, model, _plot = False, time_limit = 1000,slope_limit = 1E-8,break_param = False, params = None):
        ...
        for t in range(20):
            ...
            z = self.dX_dt(self.conc[-1],self.time_array[-1],model,minstep = 0.001)
...
def go():
    ...
    _k1 = 0.0007# Ethanol desorption
    k2 = 0.001
    #0.001#140# 5 ### Jelemensky = 10 # second dehydrogenation onto Pt (assume similar as with OH)
    k3 = 1#5#11.25 # Reaction of adosorbed ehtanol with PtOH
    k4 = 1# 2.025  # Reaction of of Ethanol with PtO
    ...
    figure()
    x = RateRunout(inits = a,parameters = (k1,_k1,k2, k3,k4 ,k5,k6,_k6 , k7,k8 ,k9 ,g ,vOads,vOH,voffset) )
    ...
    induction_time = x.time_array[argmax(diff(x.conc[:,Al]))]
    axvline(induction_time,color = 'r')
    print 'OH', x.conc[-1,OH]
    print 'O', x.conc[-1,Oads]
...
def plot_O2_dep():
    ...
    k2 = 0.001#0.001#140# 5 ### Jelemensky = 10 # second dehydrogenation onto Pt (assume similar as with OH)
    k3 = 1#5#11.25 # Reaction of adosorbed ehtanol with PtOH
    k4 = 0.01# 2.025  # Reaction of of Ethanol with PtO
    ...
    for i in O2_conc:
        ...
        x = RateRunout(inits = a,parameters = (k1,_k1,k2, k3,k4 ,k5,k6,_k6 , k7,k8 ,k9 ,g ,vOads,vOH,voffset) )
    #threading.Thread(target = x.RunOut, args = ('Jelemensky',), kwargs = {'time_limit':10,'_plot':True} ).start()
        x.RunOut('L-H with ethoxy switch',time_limit = 5000,_plot = False)
        r.append(x)
    figure()    
    subplot(311)
    ind_time = list()
    finalrate = list()
    for x in r:
        plot(x.time_array,x.conc[:,Al])
        ind_time.append(x.time_array[argmax(diff(x.conc[:,Al]))])
        finalrate.append(x.conc[-1,Al]*k5)
        
    legend(O2_conc)
    subplot(312)
    relationship = (O2_conc,ind_time)
    plot(*relationship)
    
    subplot(313)
    plot(O2_conc,finalrate)
...
def plot_k1rev_dep():
   ...
   # Ethanol desorption
    ...
    r = list()
    _k1_list = arange(0,0.001,0.0002)
    for i in _k1_list:
         x = RateRunout(inits = a,parameters = (k1,i,k2, k3,k4 ,k5,k6,_k6 , k7,k8 ,k9 ,g ,vOads,vOH,voffset) )

    #threading.Thread(target = x.RunOut, args = ('Jelemensky',), kwargs = {'time_limit':10,'_plot':True} ).start()
         x.RunOut('L-H with ethoxy switch',time_limit = 2000,_plot = False)
         r.append(x)
   # ind_time = list()
    figure()
    for x in r:
...
def plot_k2_dep():
    ...
    r = list()
    k2_list = arange(0,0.01,0.002)
    for i in k2_list:

         parameters = (k1,_k1,i, k3,k4 ,k5,k6,_k6 , k7,k8 ,k9 ,g ,vOads,vOH,voffset) 
         x = RateRunout(inits = a,parameters = parameters)
    #threading.Thread(target = x.RunOut, args = ('Jelemensky',), kwargs = {'time_limit':10,'_plot':True} ).start()
         x.RunOut('L-H with ethoxy switch',time_limit = 2000,_plot = False)
         r.append(x)
   # ind_time = list()
...
def plot_k6_dep():
    ...
    r = list()
    k_list = linspace(1E5,1E7,5)
    for i in k_list:

         parameters = (k1,_k1,k2, k3,k4 ,k5,i,_k6 , k7,k8 ,k9 ,g ,vOads,vOH,voffset) 
         x = RateRunout(inits = a,parameters = parameters)
    #threading.Thread(target = x.RunOut, args = ('Jelemensky',), kwargs = {'time_limit':10,'_plot':True} ).start()
         x.RunOut('L-H with ethoxy switch',time_limit = 2000,_plot = False)
         r.append(x)
    ...
    for x in r:
        plot(x.time_array,x.conc[:,Al])
        finalO.append(x.conc[-1,Oads]+x.conc[-1,OH])
        #ind_time.append(x.time_array[argmax(diff(x.conc[:,Al]))])
...
def plot_k6rev_dep():
   ...
   # _k6 =50# O2 des
    ...
    r = list()
    k_list = linspace(50,500,5)
    for i in k_list:

         parameters = (k1,_k1,k2, k3,k4 ,k5,k6,i , k7,k8 ,k9 ,g ,vOads,vOH,voffset) 
         x = RateRunout(inits = a,parameters = parameters)
    #threading.Thread(target = x.RunOut, args = ('Jelemensky',), kwargs = {'time_limit':10,'_plot':True} ).start()
         x.RunOut('L-H with ethoxy switch',time_limit = 2000,_plot = False)
         r.append(x)
