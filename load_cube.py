# -*- coding: utf-8 -*-
"""
Created on Thu Jan 21 15:56:44 2016
...
# The last section in the header is one line for each atom consisting of 5 numbers, the first is the atom number, second (?), the last three are the x,y,z coordinates of the atom center. 
    self.atoms = []
    atomcenter = np.array([0,0,0])
    for i in range(self.natoms):
      tkns = f.readline().split()
      self.atoms.append([tkns[0], tkns[2], tkns[3], tkns[4]])
     
      atomcenter+=np.array([float(tkns[2]), float(tkns[3]), float(tkns[4])])
    self.atomcenter = atomcenter/self.natoms
    
    self.zs = np.linspace(0,(self.NZ-1)*self.Z[2],self.NZ)
# Volumetric data
    ...
    temp = np.array(self.NX*[self.NY*[self.zs+self.origin[0]]])
    temp2 = np.transpose(temp,(0,2,1))
    temp3 = np.transpose(temp,(2,1,0))
    self.radius = np.sqrt(temp**2+temp2**2+temp3**2)
...
# can be used for integration over spherical part of the volume
  ...
  def average_distance(self):##### average distance from structure center
   
#    avgx = np.ndarray(self.data.shape)
#    avgy = np.ndarray(self.data.shape)
#    avgz = np.ndarray(self.data.shape)
#    d = np.ndarray(self.data.shape)
#    
#    
#    for p in list(itertools.product(range(51),repeat=3)):#itertools.product(range(0,51),repeat=3):
#        x,y,z = p
#        
#        avgx[x,y,z] = self.data[x,y,z]**2*(self.zs[x]+self.origin[0])
#        avgy[x,y,z] = self.data[x,y,z]**2*(self.zs[y]+self.origin[1])
#        avgz[x,y,z] = self.data[x,y,z]**2*(self.zs[z]+self.origin[2])
#       # d[x,y,z] = self.data[x,y,z]**2*np.sqrt(self.zs[x]**2+self.zs[y]**2+self.zs[z]**2)
#        
#        d[x,y,z] = self.data[x,y,z]**2*np.sqrt((self.zs[x]+self.origin[0])**2
#                        +(self.zs[y]+self.origin[1])**2
#                        +(self.zs[z]+self.origin[2])**2)
#
#
#
#    print 'average x (Angstroms)', 0.529*np.sum(avgz)/np.sum(self.data**2)
#    print 'average y (Angstroms)', 0.529*np.sum(avgx)/np.sum(self.data**2)
#    print 'average z (Angstroms)', 0.529*np.sum(avgy)/np.sum(self.data**2)
#    d = 0.529*np.sum(d)/np.sum(self.data**2)
#    print 'average distance (Angstroms)',d
    
    
    d = 0.529*np.sum(self.radius*self.data**2)/np.sum(self.data**2)
    print 'ave distance new way', d
    return d#(avgx,avgy,avgz)
        
        
        
  def overlap(self,othercube):

      return np.sum(self.data**2*othercube.data**2)/np.sqrt(np.sum(self.data**4)*np.sum(othercube.data**4))
        
#    plt.imshow(cube.data[0], cmap = cm.hot, norm = LogNorm())# extent = (np.amin(x), np.amax(x), np.amin(y), np.amax(y)),
#          
#    plt.colorbar()
#    plt.show()
